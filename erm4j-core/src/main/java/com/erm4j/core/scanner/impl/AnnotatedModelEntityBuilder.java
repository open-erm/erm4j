...
public class AnnotatedModelEntityBuilder implements ClassGraphEntityBuilder {
		...
		if (annotationInfo != null) {
			for (AnnotationParameterValue param : annotationInfo.getParameterValues()) {
				...
				if (param.getName().equalsIgnoreCase(TARGET_PARAM_NAME) && 
						(value instanceof AnnotationClassRef) ) {
					...
					attr.setForeignKey(isFK);
						...
						(value instanceof AnnotationEnumValue)) {
	...
	private String lookupTargetEntityUID(ClassInfo targetClassInfo) {
		...
		if (targetClassInfo.hasAnnotation(ENTITY_ANNOTATION_NAME)) {
			...
			if (StringUtils.isBlank(targetEntityUID)) {
				targetEntityUID = ClassInfoHelper.generateClassUID(targetClassInfo);
			}
		}
	...
	private String lookupTargetEnumUID(ClassInfo targetClassInfo) {
		...
		if (targetClassInfo.hasAnnotation(ENUMERATION_ANNOTATION_NAME)) {
			...
			if (StringUtils.isBlank(targetEnumUID)) {
				targetEnumUID = ClassInfoHelper.generateClassUID(targetClassInfo);
			}
		}
	...
	private void fillAttributeFieldsWithAutogeneratedValues(EntityAttribute attribute, ClassInfo classInfo,
		...
		if (StringUtils.isBlank(attribute.getUid())) {
			attribute.setUid(ClassInfoHelper.generateClassFieldUID(classInfo, fieldInfo));
		}
		
		if (StringUtils.isBlank(attribute.getSystemName())) {
			attribute.setSystemName(ClassInfoHelper.generateClassFieldSystemName(fieldInfo));
		}

		if (StringUtils.isBlank(attribute.getName())) {
			attribute.setName(ClassInfoHelper.generateClassFieldName(fieldInfo));
		}
		
		if (StringUtils.isBlank(attribute.getColumn())) {
			attribute.setColumn(ClassInfoHelper.generateTableColumnName(fieldInfo,tableNaming));
		}
	}
	...
	private void fillEntityFieldsWithAutogeneratedValues(Entity entity, ClassInfo classInfo,
		...
		if (StringUtils.isBlank(entity.getSystemName())) {
			entity.setSystemName(ClassInfoHelper.generateEntitySystemName(classInfo));
		}
		if (StringUtils.isBlank(entity.getName())) {
			entity.setName(ClassInfoHelper.generateEntityName(classInfo));
		}
		if (StringUtils.isBlank(entity.getTable())) {
			entity.setTable(ClassInfoHelper.generateTableName(classInfo,tableNaming));
		}
	}
